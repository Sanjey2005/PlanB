...
def is_new_user(user_phone: str) -> bool:
    """Return True if user_phone has never completed a pipeline run.

    A user is considered new when both conditions hold:
    - total_pipeline_runs == 0  (no run has ever finished for them)
    - protected_habits == []    (no habits have been learned yet)

    If S3 is unreachable, defaults are returned and the user is treated as new.
    """
    dna = get_user_dna(user_phone)
    return dna.get("total_pipeline_runs", 0) == 0 and dna.get("protected_habits", []) == []


def init_user_dna(user_phone: str) -> None:
    """Save a fresh default DNA profile to S3 for a newly onboarded user.

    Writes the default template (total_pipeline_runs=0) so the user is
    registered in S3 immediately. The automatic update_user_dna call in
    run_pipeline will increment total_pipeline_runs to 1 when the onboarding
    pipeline completes, correctly marking them as no longer new.
    """
    if not user_phone:
        return
    try:
        ...
        print(f"[UserDNA] Initialized profile for new user {user_phone}")
    except Exception as e:
        print(f"[UserDNA] Failed to initialize profile for {user_phone}: {e}")
